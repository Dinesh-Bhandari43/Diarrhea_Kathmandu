---
title: Regression model for climate variablitly and diarrhoeal disease study Kathmandu
  city, Nepal
...
### Natural cubic spline with 3 degree of freedom per year has been set to account for seasonality and long term variation###
### Play with the degree of freedom (othewise also called knots) to see the change in final qAIC value of the model###

spl <- ns(data$time, df=40)

### Compute a variable for rainfall to determine increase in estimate per 10 mm rise in precipitation##
data$rainfall10 <- data$rainfall/10

## Run a modle without including the predictors to see the effect of smoothing (null model)###
...
### Checking for partiatl autocorrelation function of the model##
...
Cbotemp<- crossbasis(data$max_temp, lag=1, argvar=list(fun="thr",
     thr.value=quantile(obs$max_temp,0.5, na.rm=T)), 
              arglag=list(fun="integer"))

cborain <- crossbasis(data$rainfall10,lag=1,argvar=list(fun="lin"),
                      arglag=list(fun="integer"))

### Runnning the regression model using the covariates###
...
pred2 <- crosspred(cbotemp, model2,at= 1, bylag=0.2)
...
plot(pred2, type="p",var=1, ci="bars",pch=19, ylim=c(0.85,1.3),main=" Distributed lag model for temperature", xlab= "Lag months", ylab="RR and 95% CI per 1 degree increase in max_temp", col="red")
